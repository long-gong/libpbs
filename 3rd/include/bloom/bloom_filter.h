...
class bloom_filter
{
 ...
 public:
  ...
  inline ssize_t set(Iterator first, Iterator last) {
    auto ssz = std::distance(first, last) ;
    if ((std::size_t)ssz != table_size_ / 8) return -1;
    std::copy(first, last, bit_table_.begin());
    return ssz;
  }
