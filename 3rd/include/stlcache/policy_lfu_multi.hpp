...
namespace stlcache {
template<class Key, template<typename T> class Allocator>
class _policy_lfu_multi_type : public policy<Key, Allocator> {
 protected:
  struct Reference {
    Key key;
 ...
 public:
  ...
  _policy_lfu_multi_type<Key, Allocator> &operator=(const _policy_lfu_multi_type<Key, Allocator> &x) {
#else
...
#if __cplusplus >= 201703L
  _policy_lfu_multi_type(const _policy_lfu_multi_type<Key, Allocator> &x) {
#else
